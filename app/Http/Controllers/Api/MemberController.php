...
class MemberController extends Controller
{
        ...
        if ($memberPhone) {
            ...
            return response()->json([
                 "status" => "validationfail",
                  ...
                  "message" => "{$memberPhone->phoneNumber} ফোন নম্বর আগে থেকেই আছে", //
            ]);
        }
    ...
    public function login(Request $request)
    {
        ...
        $user = Member::where('phoneNumber', $request->phoneNumber)->first();
        $credentials = [
        ...
        // return $credentials;
        try {
            if (!($token = Auth::guard("memberapi")->attempt($credentials))) {
                return response()->json(
                    [
    ...
    public function getProfileInfo(Request $request) {
        ...
        // $memberInfo = Member::findOrFail(Auth::guard('member')->user()->id);
        $memberInfo = Member::where('id', $request->id)->select('id','fullName','phoneNumber','image','status')->first();
        // return $memberInfo;
        $basicInformation = BasicInfo::where('member_id', $memberInfo->id)->select('id','present_address', 'marital_status', 'nationality_id', 'country_id', 'complexion', 'religion_id', 'age','feet','inch',)->with('maritalstatus', 'religion', 'country', 'nationality', 'pcomplexion')->first();
        if ($basicInformation) {
            $memberEdit = Member::where('id',$request->id)->first();
            $memberEdit->gender = $basicInformation->gender;
            $memberEdit->save();
        }
        $educationcareer = EducationCareer::where('member_id', $memberInfo->id)->with('working', 'profession')->first();
        ...
        $educationalvalues = EducationValue::orderBy('degree_id', 'desc')->where('member_id', $memberInfo->id)->with('degree', 'education')->first();
        $familylocation = FamilyLocation::where('member_id', $memberInfo->id)->select('id', 'member_id', 'father_name','mother_name','alt_contact', 'division_id', 'district_id','upazila_id', 'permanent_address','present_address',)->with('division', 'district', 'upazila')->first();
        $present_upazillas = Location::orderBy('id', 'ASC')->where(['district_id' => $familylocation ? $familylocation->present_district : ''])->get();
