...
<div class="container-fluid">
    ...
    <!-- end page title --> 
    <form action="{{ route('location.assign_division') }}" method="POST">
        @csrf
    <div class="row">
        <div class="col-sm-12 d-flex">
            <div class="form-group" style="width: 250px; margin-right: 15px">
                 <select class="form-control select2-multiple" name="division_id" data-toggle="select2"  data-placeholder="Choose ...">
                    <optgroup >
                        <option value="">Select..</option>
                        @foreach($data_divisions as $value)
                        <option @if(Session::get('next_id_2') == $value->id) selected @endif value="{{$value->id}}">{{$value->title}}</option>
                        @endforeach
                    </optgroup>
                </select>
                @error('division_id')
